...
async function loadBlackjackMinigame() {
    ...
    if (window.testingMode) {
        ...
        if (!document.getElementById('testing-mode-banner')) {
            const banner = document.createElement('div');
            banner.id = 'testing-mode-banner';
            banner.textContent = 'Testing Mode: Click the hotel icon (bottom right) to switch minigame.';
            banner.style.position = 'fixed';
            banner.style.top = '12px';
            banner.style.right = '50%';
            banner.style.transform = 'translateX(50%)';
            banner.style.background = 'rgba(34,34,34,0.92)';
            banner.style.color = '#FFD700';
            banner.style.fontSize = '1em';
            banner.style.padding = '6px 18px';
            banner.style.borderRadius = '8px';
            banner.style.zIndex = '4000';
            banner.style.boxShadow = '0 2px 8px #0006';
            banner.style.pointerEvents = 'none';
            document.body.appendChild(banner);
        }
        // Add a floating hotel/minigame selector button if not already present
        if (!document.getElementById('testing-mode-hotel-btn')) {
            const btn = document.createElement('button');
            btn.id = 'testing-mode-hotel-btn';
            btn.title = 'Switch Minigame';
            btn.innerHTML = '🏨';
            ...
            btn.onclick = () => {
                ...
                if (!selector) {
                    ...
                    selector.id = 'testing-mode-hotel-selector';
                    selector.style.position = 'fixed';
                    selector.style.bottom = '90px';
                    selector.style.right = '32px';
                    selector.style.background = 'rgba(34,34,34,0.98)';
                    selector.style.color = '#FFD700';
                    selector.style.padding = '10px 18px';
                    ...
                    hotelMinigameMap.forEach(([hotel, minigame]) => {
                        ...
                        hBtn.style.background = '#FFD700';
                        hBtn.style.color = '#222';
                        hBtn.style.border = 'none';
                        hBtn.style.borderRadius = '6px';
                        hBtn.style.fontSize = '1em';
                        hBtn.style.cursor = 'pointer';
                        hBtn.onclick = () => {
                            selector.remove();
                            onPropertyUILoaded(hotel);
                        };
