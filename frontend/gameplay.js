// --- Modern Slot Machine Minigame ---
const SLOT_SYMBOLS = [
    { name: 'cherry', color: '#ff1744', emoji: '🍒', payout: 5 },
    { name: 'lemon', color: '#fff44f', emoji: '🍋', payout: 4 },
    { name: 'bar', color: '#fff', emoji: '🟫', payout: 10 },
    { name: 'seven', color: '#ff9100', emoji: '7️⃣', payout: 20 },
    { name: 'bell', color: '#ffd700', emoji: '🔔', payout: 8 },
    { name: 'grape', color: '#8e24aa', emoji: '🍇', payout: 6 }
];
const SLOT_REELS = 3;
const SLOT_ROWS = 3;
...
function showSlotMachineGame() {
    const slotDiv = document.getElementById('slotmachine-game');
    if (!slotDiv) return;
    slotDiv.style.display = 'flex';
    renderSlotMachine([0,1,2], [0,1,2], [0,1,2]);
    document.getElementById('slotmachine-result').textContent = '';
    document.getElementById('slotmachine-spin-btn').onclick = spinSlotMachine;
}

function hideSlotMachineGame() {
    const slotDiv = document.getElementById('slotmachine-game');
    if (slotDiv) slotDiv.style.display = 'none';
}

function renderSlotMachine(r0, r1, r2) {
    const canvas = document.getElementById('slotmachine-canvas');
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0,0,400,320);
    // Draw reels
    for (let c=0; c<SLOT_REELS; c++) {
        for (let r=0; r<SLOT_ROWS; r++) {
            const idx = [r0, r1, r2][c][r];
            const sym = SLOT_SYMBOLS[idx];
            ctx.save();
            ctx.translate(70 + c*130, 60 + r*80);
            ctx.beginPath();
...
function spinSlotMachine() {
    ...
    const bet = parseInt(document.getElementById('slotmachine-bet').value) || 1;
    let reels = [[],[],[]];
    for (let c=0; c<SLOT_REELS; c++) {
        for (let r=0; r<SLOT_ROWS; r++) {
            reels[c][r] = Math.floor(Math.random()*SLOT_SYMBOLS.length);
        }
    }
    ...
    function animateSpin() {
        ...
        renderSlotMachine(reels[0], reels[1], reels[2]);
        anim++;
        if (anim < spins) {
            setTimeout(animateSpin, 60 + anim*2);
        } else {
            slotmachineSpinning = false;
            slotmachineResult = [reels[0], reels[1], reels[2]];
            showSlotMachineResult(bet, reels);
        }
    }
    animateSpin();
}

function showSlotMachineResult(bet, reels) {
    // Only pay for center row
    const row = 1;
    const s0 = reels[0][row], s1 = reels[1][row], s2 = reels[2][row];
    let payout = 0;
    let msg = '';
    if (s0 === s1 && s1 === s2) {
        payout = bet * SLOT_SYMBOLS[s0].payout;
        msg = `JACKPOT! ${SLOT_SYMBOLS[s0].emoji} x3 +$${payout}`;
    ...
    document.getElementById('slotmachine-result').textContent = msg;
}

// For demo: expose to window
window.showSlotMachineGame = showSlotMachineGame;
window.hideSlotMachineGame = hideSlotMachineGame;
// --- Modern Roulette Minigame ---
// American roulette wheel order and colors
const ROULETTE_NUMBERS = [0, '00', 27, 10, 25, 29, 12, 8, 19, 31, 18, 6, 21, 33, 16, 4, 23, 35, 14, 2, 0, 28, 9, 26, 30, 11, 7, 20, 32, 17, 5, 22, 34, 15, 3, 24, 36, 13, 1, '00'];
const ROULETTE_COLORS = ["green","green","red","black","red","black","red","black","red","black","red","black","red","black","red","black","red","black","red","black","green","black","red","black","red","black","red","black","red","black","red","black","red","black","red","black","red","black","red","green"];
...
function renderRouletteWheel() {
    ...
    for(let i=0;i<n;i++){
        ...
        ctx.beginPath();
        ctx.moveTo(200,200);
        ctx.arc(200,200,180,start,end,false);
        ctx.lineTo(200,200);
        ctx.closePath();
        ...
        ctx.fillStyle = (ROULETTE_COLORS[i]==='black'||ROULETTE_COLORS[i]==='red')?'#fff':'#222';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(ROULETTE_NUMBERS[i], 150, 0);
        ctx.restore();
    }
    // Center
    ctx.beginPath();
...
let rouletteBet = { type: 'straight', number: 17, amount: 10 };

function renderRouletteTable() {
    // For now, just show a simple straight bet UI. Expand later for full table.
    const tableDiv = document.querySelector('.roulette-table');
        // Render a real roulette table with red/black/green clickable bets
        tableDiv.innerHTML = `<div style="margin-bottom:12px;font-weight:bold;">Place Your Bet</div>
            <div style="display:grid;grid-template-columns:repeat(3,40px);gap:4px;margin-bottom:10px;">
                ${Array.from({length: 12}, (_,row) =>
                    Array.from({length: 3}, (_,col) => {
                        const num = row*3+col+1;
                        const idx = ROULETTE_NUMBERS.indexOf(num);
                        const color = ROULETTE_COLORS[idx];
                        return `<button class="roulette-bet-btn" data-num="${num}" style="background:${color};color:${color==='red'||color==='black'?'#fff':'#222'};font-weight:bold;border-radius:6px;border:2px solid #ffd700;font-size:1.1em;">${num}</button>`;
                    }).join('')
                ).join('')}
            </div>
            <div style="display:flex;gap:8px;margin-bottom:10px;">
                <button class="roulette-bet-btn" data-num="0" style="background:green;color:#fff;font-weight:bold;border-radius:6px;border:2px solid #ffd700;font-size:1.1em;">0</button>
                <button class="roulette-bet-btn" data-num="00" style="background:green;color:#fff;font-weight:bold;border-radius:6px;border:2px solid #ffd700;font-size:1.1em;">00</button>
            </div>
            <label>Amount: <input id="roulette-bet-amount" type="number" min="1" value="10" style="width:60px;"></label>
            <div id="roulette-bet-selected" style="margin-top:8px;color:#ffd700;font-weight:bold;"></div>`;
        // Add click handlers for bet buttons
        tableDiv.querySelectorAll('.roulette-bet-btn').forEach(btn => {
            btn.onclick = () => {
                const num = btn.getAttribute('data-num');
                const amt = parseInt(document.getElementById('roulette-bet-amount').value);
                rouletteBet = { type: 'straight', number: num === '00' ? '00' : parseInt(num), amount: amt };
                document.getElementById('roulette-bet-selected').textContent = `Bet set: $${amt} on ${num}`;
            };
        });
}

function spinRoulette() {
    // Animate ball spin and pick a random number
    const n = ROULETTE_NUMBERS.length;
    const resultIdx = Math.floor(Math.random()*n);
    const winNumber = ROULETTE_NUMBERS[resultIdx];
    const winColor = ROULETTE_COLORS[resultIdx];
    animateRouletteBall(resultIdx, () => {
        // Show result and payout
        let payout = 0;
        if (rouletteBet.type==='straight' && winNumber===rouletteBet.number) payout = rouletteBet.amount*35;
        const resultDiv = document.getElementById('roulette-result');
        if (payout>0) {
            resultDiv.textContent = `WIN! Number: ${winNumber} (${winColor}) +$${payout}`;
        } else {
...
function animateRouletteBall(targetIdx, cb) {
    const canvas = document.getElementById('roulette-wheel');
    const ctx = canvas.getContext('2d');
    let angle = 0;
    let speed = 0.35 + Math.random()*0.1;
    let decel = 0.985 + Math.random()*0.004;
    const n = ROULETTE_NUMBERS.length;
    ...
    function drawBall(a) {
        ...
        // Ball
        const r = 170;
        const bx = 200 + r*Math.cos(a-baseAngle);
        const by = 200 + r*Math.sin(a-baseAngle);
        ctx.beginPath();
    ...
    function anim() {
        ...
        angle += speed;
        speed *= decel;
        if (speed < 0.012 && Math.abs((angle%(2*Math.PI)) - targetAngle) < 0.09) {
            drawBall(targetAngle);
            setTimeout(cb, 700);
            return;
        }
...
// For demo: expose to window
window.showRouletteGame = showRouletteGame;
window.hideRouletteGame = hideRouletteGame;
