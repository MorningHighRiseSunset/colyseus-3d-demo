// --- Roulette Game Logic & Animation (Modular) ---
window.initRouletteMinigame = function(container) {
	// --- DOM helpers ---
...
// end drawWheel
	...
	container.querySelectorAll('.bet-box').forEach(box => {
	...
	container.querySelectorAll('.column-bet').forEach((col, i) => {
	...
	function highlightBet(num) {
		...
		container.querySelectorAll(`[aria-label="${num}"]`).forEach(cell => {
	...
	function updatePlayerInfo() {
		...
		let betsList = Object.keys(bets || {}).length ? Object.entries(bets).map(([k,v]) => `${k} ($${v*betAmount})`).join(', ') : 'No bets placed yet';
		let remaining = playerBalance - totalBets * betAmount;
		const balElem = q('#player-balance');
		const betsElem = q('#player-bets');
		const remElem = q('#player-remaining');
		if (balElem) balElem.textContent = `$${playerBalance}`;
	...
	container.querySelectorAll('.column-bet').forEach(box => {
		box.addEventListener('click', updatePlayerInfo);
	});

	// --- Instructions Modal ---
	const instrBtn = q('#instructions-btn');
	const instrModal = q('#instructions-modal');
	const closeInstr = q('#close-instructions');
	instrBtn.onclick = () => { instrModal.style.display = 'flex'; };
	...
	function showResult() {
		...
		if (win) {
			playerBalance += Object.values(bets || {}).reduce((a,b) => a+b, 0) * betAmount * 2; // Simple payout: 2x
			msg += ' | You win!';
		} else {
			playerBalance -= Object.values(bets || {}).reduce((a,b) => a+b, 0) * betAmount;
			msg += ' | No win.';
		}
		updatePlayerInfo();
		// Show popup/modal for result
		let resultModal = container.querySelector('#result-modal');
		if (!resultModal) {
		...
		let inner = container.querySelector('#result-modal-inner');
		inner.innerHTML = `<h2>${win ? 'You Win!' : 'No Win'}</h2><p>${msg}</p><div style='margin:12px 0;font-weight:bold;'>Money Available: <span id='modal-player-balance'>$${playerBalance}</span></div><button id="close-result-modal" style="margin-top:18px;padding:8px 22px;font-size:1em;background:#e2b07a;color:#222;border-radius:10px;border:none;box-shadow:0 2px 8px #0005;cursor:pointer;font-weight:bold;">OK</button>`;
		resultModal.style.display = 'flex';
		inner.querySelector('#close-result-modal').onclick = function() {
